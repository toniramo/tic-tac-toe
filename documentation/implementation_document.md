...
- Used heuristics simplify the play style of AI, at least rule 'iterate through only those tiles that are adjacent to reserved tiles'. This could be revised and changed to something more complex since it is possible that sometimes the most optimal move is found elsewhere. Of course, abandoning this rule as it is now may require improving the performance of the algortihm in some other means to compensate the change.
- It would be nice to introduce more variety in the way AI picks the moves. I.e. if there are two or more moves that are considered equal by used heuristics, AI could choose between those randomly. Now equally valuable move that is found last is chosen resulting in always same game trees between two AIs. Luckily, letting user choose the first move in AI vs. AI game brings at least some variety when different starting locations are selected.
